...
2. The next step is to enable clustering. **Latency is critical** for forming well-defined clusters (So set latency.sh file and run).

* **Wait 30+ minutes** (or longer) until the Serf RTT coordinates are fully populated. You can monitor this with `./serf rtt serf12` command and check for different nodes.
* Once the RTT are correctly given by the above serf command for all nodes then in any container (preferably **serf1**, which is the most commonly used), go to: `/opt/serfapp/`
* Open `setup.txt` and adjust the **latency threshold** as needed.
* Run `bootstrap.py`.

When `bootstrap.py` prompts you:

* **Server IP address:** enter the **eth1 IP** of the **serf1** container.
* **Port:** just press **Enter** to accept the default.
* When prompted for the next value, enter **100** and press **Enter**.

While clustering is running:

* In another terminal on **serf1**, follow the log file to confirm progress: `/var/log/serfapp_bg.log`

After clustering completes:

* Run `./serf members` to verify the newly created clusters.

3. If the pods are **not running** on any node, run:
   ```bash
...
4. To check if all pods are running on all nodes:
   ```bash
...
```bash
./liqo_install/Workload_Offloading_API/setup_liqo_api.sh
```
